              ...
              <div class="crowdfunding-info">
                ...
                <div>此项目必须在
                  ...
                  的支持才可以成功，
                   ...
                   筹款将在<span class="text-red">{{ $product->crowdfunding->end_at->diffForHumans() }}</span>结束！
                </div>
                @endif
              </div>
            @else
              ...
              @elseif ($product->type === \App\Models\Product::TYPE_SECKILL)
                @if (Auth::check())
                  ...
                  @elseif ($product->seckill->is_after_end)
                    <button class="btn btn-primary btn-seckill disabled">抢购结束</button>
                  @else
                ...
                <div class="col-3 product-item">
                  <div class="product-content">
                    <div class="top">
                      ...
                      <div class="title">
                        <a href="{{ route('products.show', $p->id) }}">{{ $p->title }}</a>
                      </div>
                    </div>
                  </div>
                </div>
...
<!-- 如果是秒杀商品并且尚未开始秒杀，则引入 moment.js 类库 -->
...
<script>
  $(document).ready(function () {
    ...
    $('.btn-seckill').click(function () {
      ...
      swal({
        text: '选择收货地址',
        // addressSelector 是一个 jQquery 对象，addressSelector[0] 获取其第一个 dom 对象
