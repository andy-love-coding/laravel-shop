        ...
        <div class="col-md-9">
          <div class="form-row">
            ...
            <div class="col-auto category-breadcrumb">
              ...
              @endforeach
              <!-- 最后展示出当前类目名称 -->
                <span class="category">{{ $category->name }}</span> ><span>
                <!-- 当前类目的 ID，当用户调整排序方式时，会以表单值的形式提交 category_id，可以保证 category_id 参数不丢失 -->
...
@section('scriptsAfterJs')
  <script>
    ...
    function parseSearch() {
      // 初始化一个空对象
      var searches = {};
      // location.search 会返回 Url 中 ？及以后的查询参数
      ...
      location.search.substr(1).split('&').forEach(function(str) {
        ...
        if(decodeURIComponent(result[0]) && decodeURIComponent(result[1])){
    ...
    function appendFilterToQuery(name, value) {
      ...
      // 如果已经有了 filters 查询
      if (searches['filters'] && searches['filters'] !== 'undefined') {
        // 则在已有的 filters 后追加
