...
class InstallmentsController extends Controller
{
    ...
    public function payByAlipay(Installment $installment)
    {
        ...
        if (!$nextItem = $installment->items()->whereNull('paid_at')->orderBy('sequence')->first()) {
            // 如果没有支付的还款，原则上不可能，因为如果分期已经结清则在上一个判断就退出了
            throw new InvalidRequestException('该分期订单已结清');
        }
        // 调起支付宝的网页支付
    ...
    public function alipayNotify()
    {
        ...
        // 如果订单没有成功或者结束，则不走后续的逻辑
        if (!in_array($data->trade_status, ['TRADE_SUCCESS', 'TRADE_FINISHED'])) {
            return app('alipay')->success();
        }
        // 拉起支付时使用的支付订单号是由 分期流水号+还款计划编号 组成的
        // 因此可以通过支付订单号来还原出这笔还款是哪个分期付款的那个还款计划
        list($no, $sequence) = explode('_', $data->out_trade_no);
        // 根据分期流水号查询对应的分期记录，原则上不会找不到，这里的判断只是增加代码健壮性
        if (!$installment = Installment::where('no', $no)->first()) {
            return 'fail';
        }
        // 根据还款计划编号，查询对应的还款计划，原则上不会找不到，这里的判断只是增加代码的健壮性
        if (!$item = $installment->items()->where('sequence', $sequence)->first()) {
            return 'fail';
        }
        // 如果这个还款计划的支付状态是已支付，则告知只补办此订单已完成，并不在执行后续逻辑
        if ($item->paid_at) {
            return app('alipay')->success();
        }
